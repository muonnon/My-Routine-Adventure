...
import javax.swing.table.DefaultTableCellRenderer; //--251119
import java.awt.*;
...
import java.time.DayOfWeek; //--251119
import java.time.LocalDate; //251119
	...
	private JTabbedPane createTabbedPane() {
		...
		for (String day : DAYS) {
			...
			DefaultTableModel model = new DefaultTableModel(TABLE_HEADERS, 0) {
				...
				//251119: 미래 요일 및 과거 요일 편집 방지 (오늘만 체크 가능)
				@Override
				...
				if (column == 0) {
					...
					if (isTooPastDay(this.day)) {
						JOptionPane.showMessageDialog(RoutineListView.this,
								this.day + "요일은 2일 이상 지난 과거 요일이므로 루틴을 완료할 수 없습니다.\n(어제까지만 체크 가능합니다)", "경고",
